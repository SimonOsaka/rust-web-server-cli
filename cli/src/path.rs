...
impl Path {
    pub(crate) fn new() -> Self {
        Path {
            ...
            file_api_name: FileName { old_name: "api".to_string(), new_name: "api".to_string() },
            file_domain_name: FileName{old_name:"domain".to_string(),new_name:"domain".to_string()},
            file_repository_name: FileName{old_name:"repository".to_string(),new_name:"repository".to_string()},
            file_types_name: FileName{old_name:"types".to_string(),new_name:"types".to_string()},
        }
    }
    pub(crate) fn rename_dir(&self, path: &str) -> String {
        //api
        let example_api_path_old = format!("{}/{}", &self.example_path, &self.file_api_name.old_name);
        let example_api_path_new = format!("{}/{}", &self.example_path, &self.file_api_name.new_name);
        //domain
        let example_domain_path_old = format!("{}/{}", &self.example_path, &self.file_domain_name.old_name);
        let example_domain_path_new = format!("{}/{}", &self.example_path, &self.file_domain_name.new_name);
        //repository
        let example_repository_path_old =
            format!("{}/{}", &self.example_path, &self.file_repository_name.old_name);
        let example_repository_path_new =
            format!("{}/{}", &self.example_path, &self.file_repository_name.new_name);
        //types
        let example_types_path_old = format!("{}/{}", &self.example_path, &self.file_types_name.old_name);
        let example_types_path_new = format!("{}/{}", &self.example_path, &self.file_types_name.new_name);
        let rename_dir;
